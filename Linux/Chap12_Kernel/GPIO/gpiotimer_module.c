...
#define GPIO_LED        589 //18
#define GPIO_SW         595 //24
...
static int switch_irq;  //GPIOIRQ
static struct timer_list timer;
static DEFINE_MUTEX(led_mutex); //GPIOTIMER
...
int init_module(void)
{
  ...
  gpio_direction_input(GPIO_SW);    //added
...
static ssize_t gpio_write(struct file * inode , const char * buff, size_t len, loff_t *off)
{
  ...
  printk("GPIO Device(%d) write :%s(%zu)\n", MAJOR(inode->f_path.dentry->d_inode->i_rdev), msg, (long)len);
  return count;
}
